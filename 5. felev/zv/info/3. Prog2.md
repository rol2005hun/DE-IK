...
<details>
<summary><b>🛠️ Konstruktor, Destruktor, Függőség</b></summary>

### Konstruktor
A konstruktor egy speciális metódus, amelyet az objektum példányosításakor automatikusan meghív a rendszer. Fő feladata az objektum inicializálása, a tagváltozók kezdőértékének beállítása.

**Főbb jellemzők:**
- Neve megegyezik az osztály nevével (Java, C++), nincs visszatérési értéke.
- Több konstruktor is lehet (konstruktor túlterhelés).
- Statikus tagokat nem inicializál, csak példánytagokat.
- Java-ban: ha nincs definiálva, automatikusan létrejön egy default konstruktor.
...
class Pont {
    int x, y;
    Pont() { x = 0; y = 0; }
    Pont(int x, int y) { this.x = x; this.y = y; }
}
Pont p1 = new Pont();
...
**Advanced magyarázat:**
- Konstruktor láncolás: egyik konstruktor meghívja a másikat (`this(...)`)
- Immutábilis objektumok: csak konstruktorban állíthatóak be a mezők
- Dependency Injection: konstruktoron keresztül adunk át függőségeket

### Destruktor
A destruktor egy speciális metódus, amelyet az objektum életciklusának végén hív meg a rendszer. Fő feladata az erőforrások felszabadítása (memória, fájl, hálózat stb.).

**Főbb jellemzők:**
- C++: `~OsztalyNev()` szintaxissal, automatikusan hívódik, nem hívható kézzel.
- Java: nincs klasszikus destruktor, helyette a `finalize()` metódus (de nem ajánlott, helyette try-with-resources, vagy explicit felszabadítás).
- Python: `__del__` metódus.
...
class File {
    FILE* f;
public:
    File(const char* nev) { f = fopen(nev, "r"); }
    ~File() { if (f) fclose(f); }
};
```

**Advanced magyarázat:**
- RAII (Resource Acquisition Is Initialization): C++-ban a konstruktor szerzi meg, a destruktor szabadítja fel az erőforrást.
- Java-ban: erőforrásokat explicit kell kezelni (pl. `close()` metódus), garbage collector nem garantálja az azonnali felszabadítást.

### Függőség (Dependency)
A függőség azt jelenti, hogy egy osztály vagy objektum más osztályokra, objektumokra, vagy szolgáltatásokra támaszkodik a működéséhez.

**Főbb jellemzők:**
- Függőségek lehetnek: más osztály példányai, interfészek, szolgáltatások, konfigurációk.
- Erős (tight coupling): konkrét osztályra hivatkozás, nehezebb tesztelni, módosítani.
- Gyenge (loose coupling): interfészek, absztrakciók használata, könnyebb tesztelni, cserélni.
- Dependency Injection: a függőségeket kívülről adjuk át (konstruktor, setter, vagy framework segítségével).

**Példa (Java, konstruktoros DI):**
```java
interface Logger {
    void log(String msg);
}
...
class Service {
    ...
    public void doWork() { logger.log("Munka kész!"); }
}
...
**Advanced magyarázat:**
- Inversion of Control: a vezérlés átadása a keretrendszernek, amely beadja a függőségeket
- Singleton, Factory, Service Locator minták a függőségek kezelésére
- Tesztelhetőség: mock objektumok, tesztbarát architektúra

**Tipikus hibák:**
- Erős összekapcsolás: nehéz karbantartani, tesztelni
- Körkörös függőség: két osztály egymásra hivatkozik, nehéz inicializálni
- Destruktor hiánya: erőforrás-szivárgás

**Vizsgatipp:**
- Mutass be konstruktor túlterhelést, destruktor használatot (C++), és dependency injection példát!
- Magyarázd el, miért előnyös a laza kapcsolás!
