...
flowchart TD
    dev["/dev"]
    dev --> sda["sda (blokkeszköz)"]
    dev --> tty["tty (karakteres eszköz)"]
    dev --> null["null (speciális)"]
    dev --> zero["zero (speciális)"]
    dev --> random["random (speciális)"]
    dev --> fifo["mypipe (FIFO)"]
    dev --> socket["mysocket (Socket)"]
```
