...
A web a HTTP protokollon keresztül kommunikáló kliensek (böngészők) és szerverek rendszere. A böngésző URI-t kér, a szerver HTTP válasszal felel. A web működése TCP/IP protokollra épül, a HTTP/HTTPS a leggyakoribb alkalmazási réteg. A böngésző DNS segítségével feloldja a domain nevet, majd TCP kapcsolatot létesít a szerverrel. A HTTP protokoll stateless, ezért a munkamenetkezeléshez sütiket vagy session-t használnak. A modern weben REST API-k, WebSocket, GraphQL is elterjedt. A böngésző a szerver válaszát feldolgozza, rendereli a HTML-t, letölti a CSS-t, JS-t, képeket, majd megjeleníti az oldalt.

**Webes protokollok:**
- HTTP/1.1: keep-alive, chunked transfer
- HTTP/2: multiplexálás, fejléc tömörítés
- HTTPS: titkosított kommunikáció (TLS/SSL)
...
- Használj HTTPS-t mindenhol!
- Állítsd be a helyes Content-Type fejlécet.
- Teszteld az oldalt több böngészőben, eszközön.

**Tipikus hibák:**
- Hibás URI
- Nem megfelelő HTTP metódus
- Hiányzó vagy hibás állapotkód
...
- CORS (Cross-Origin Resource Sharing) hibák
...
**Advanced példa:**
A böngésző a https://example.com/index.html URI-t kéri, a szerver 200 OK válasszal és HTML tartalommal felel.
```http
GET /api/data HTTP/2
Host: example.com
